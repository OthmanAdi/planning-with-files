...
| `agent/session-start` with source `compact` | Injects the compaction note (plan pointer, flush instruction, attestation hash) and the framed plan so the continuation resumes at the current phase |
