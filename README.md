...
| Version | Highlights |
|---------|------------|
| **v3.10.0** | **BREAKING: removed the `-ar`/`-de`/`-es`/`-zh`/`-zht` language-variant skills** (closes #130). The 5 variants only ever differed from the canonical skill in prose, and had quietly drifted out of feature parity — missing 12 script files and the v3.2.0 Windows encoding fix. `/plan-ar`, `/plan-de`, `/plan-es`, `/plan-zh`, and `/plan-zht` are unaffected; they now invoke the single canonical skill with an inline language instruction. Anyone who installed a language skill directly by id should reinstall the base `planning-with-files` skill. |
